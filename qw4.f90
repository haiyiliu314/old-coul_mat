program main
  implicit none
  integer, parameter                        ::Na = 57, Nb = 55 , Nc = 50 , N = Na + Nb + Nc, N1 = 45  , N2=84
  ...
  integer                                   ::INFO, LWORK,i,j=0, i1=0, j2=0
  double precision                          ::RWORK(2*N), y( N )=0, y1( N ),s2( N ), y2(N2)
  ...
  double precision, dimension(N2)           ::a2
  ...
  do i= ( Na + 1 ), Na + Nb
    y(i) = dNa * Na + dNb * ( i - Na - 0.5d0);
  end do
  do i = ( Na + Nb + 1 ), N
    y(i) = dNa * Na + dNb * Nb + dNc * (i-Na-Nb-0.5d0);
  end do
!  y=y
  y1=y
  LWORK = -1
